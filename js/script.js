...
// ---------------- responsivo: escala a árvore (min-width 1500px) para caber na tela ----------------
const TREE_NATIVE_W = 1500, TREE_MIN_SCALE = 0.5;
function fitTree(){
  const outer = document.querySelector('.scroll-outer');
  const frame = document.getElementById('treeFrame');
  const wrap = document.getElementById('treeWrap');
  const available = Math.max(outer.clientWidth - 4, 260);
  if (available >= TREE_NATIVE_W){
    wrap.style.transform = 'none';
    frame.style.width = '';
    frame.style.height = '';
    return;
  }
  const scale = Math.max(available / TREE_NATIVE_W, TREE_MIN_SCALE);
  wrap.style.transform = `scale(${scale})`;
  frame.style.width = Math.round(wrap.offsetWidth * scale) + 'px';
  frame.style.height = Math.round(wrap.offsetHeight * scale) + 'px';
}
...
function drawConnectors(){
  ...
  const renderScale = wrapRect.width / wrap.offsetWidth;
  ...
  function pt(el, side){
    ...
    const x0 = (r.left - wrapRect.left) / renderScale, y0 = (r.top - wrapRect.top) / renderScale;
    const w = r.width / renderScale, h = r.height / renderScale;
    if (side === 'bottom') return { x: x0 + w/2, y: y0 + h };
    if (side === 'top')    return { x: x0 + w/2, y: y0 };
    if (side === 'left')   return { x: x0, y: y0 + h/2 };
    return { x: x0 + w, y: y0 + h/2 }; // right
  }
